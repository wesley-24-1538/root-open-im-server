...
func (m *MessageApi) SendMessage(c *gin.Context) {
	...
	sendMsgReq.MsgData.Ip = c.ClientIP()

	// 消息来源 msgFrom: 100用户消息, 200系统消息
	switch sendMsgReq.MsgData.ContentType {
	case 101, 106, 114, 102, 103, 104, 108: // 文本(文本|@|引用回复)|图片|音频|视频|个人名片
		sendMsgReq.MsgData.MsgFrom = constant.UserMsgType
		break
	case 110: // 自定义 红包|动画表情|内置动画表情
		customType, _ := utils.CustomType(sendMsgReq.MsgData)
		dataTypes := make([]int64, 0)
		dataTypes = append(dataTypes, 1100, 1201, 1207)
		if utils.IsContainInt64(customType, dataTypes) {
			sendMsgReq.MsgData.MsgFrom = constant.UserMsgType
		}
		break
	}
...
// GetConversation 根据消息ID获取会话ID
func (m *MessageApi) GetConversation(c *gin.Context) {
	a2r.Call(msg.MsgClient.MsgIdGetConversations, m.Client, c)
}
