...
func NewGinRouter(discov discoveryregistry.SvcDiscoveryRegistry, rdb redis.UniversalClient) *gin.Engine {
	...
	//ALB 检测路由
	r.HEAD("/slb/check", func(c *gin.Context) {
	...
	friendRouterGroup := r.Group("/friend", ParseToken)
	{
		...
		friendRouterGroup.POST("/get_friend_page_list", f.GetFriendPageList)
		...
		friendRouterGroup.POST("/get_friends_remark", f.GetFriendsRemark)
	...
	groupRouterGroup := r.Group("/group", ParseToken)
	{
		...
		groupRouterGroup.POST("/get_group_member_page_list", g.GetGroupMemberPageList)
		...
		groupRouterGroup.POST("/get_find_groups", g.GetFindGroups)
		groupRouterGroup.POST("/search_group_members", g.SearchGroupMembers)
	}
	...
	msgGroup := r.Group("/msg", ParseToken)
	{
		...
		//根据消息ID获取会话ID
		msgGroup.POST("/get_conversation", m.GetConversation)
	}
	// Conversation
