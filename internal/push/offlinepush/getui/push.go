...
func (g *Client) Push(ctx context.Context, userIDs, onlineUserIDs []string, title, content string, opts *offlinepush.Opts) error {
	if len(userIDs) > 0 {
		// 在线推送 | 不推ios
		err1 := g.push(ctx, userIDs, title, content, opts, true)
		if err1 != nil {
			log.ZError(ctx, "TuiPush online err", err1)
			return err1
		}
	}
	...
	if len(onlineUserIDs) > 0 {
		// 离线推送
		err2 := g.push(ctx, onlineUserIDs, title, content, opts, true)
		if err2 != nil {
			log.ZError(ctx, "TuiPush online err", err2)
			return err2
		}
	}
	return nil
...
func (g *Client) push(ctx context.Context, userIDs []string, title, content string, opts *offlinepush.Opts, pushIOS bool) error {
	...
	pushReq := newPushReq(title, content, opts.Ex)
	pushReq.setPushChannel(title, content, opts.Ex, pushIOS)
...
func (g *Client) request(ctx context.Context, url string, input any, token string, output any) error {
	...
	resp := &Resp{}
	resp.Data = output
	err := g.postReturn(ctx, config.Config.Push.GeTui.PushUrl+url, header, input, resp, 3)
	return err
}
