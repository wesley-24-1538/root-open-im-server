...
func Start(client discoveryregistry.SvcDiscoveryRegistry, server *grpc.Server) error {
	...
	gs.Notification = notification.NewGroupNotificationSender(database, &msgRpcClient, &userRpcClient, func(ctx context.Context, userIDs []string) ([]notification.CommonUser, error) {
		...
		usersWaitGroup, err := userRpcClient.GetUsersInfoWaitGroup(ctx, userIDs)
		if err != nil {
			return nil, err
		}
		return utils.Slice(usersWaitGroup, func(e *sdkws.UserInfo) notification.CommonUser { return e }), nil
	})
	gs.conversationRpcClient = conversationRpcClient
	gs.msgRpcClient = msgRpcClient
	gs.FriendRpcClient = friendRpcClient
	gs.msgCache = cache.NewMsgCacheModel(rdb)
...
func (s *groupServer) CreateGroup(ctx context.Context, req *pbgroup.CreateGroupReq) (*pbgroup.CreateGroupResp, error) {
	...
	if req.GroupInfo.GroupName == "" {
		return nil, errors.New("请输入群名称")
	}
	if req.GroupInfo.FaceURL == "" {
		return nil, errors.New("请输入群头像")
	}
	if req.OwnerUserID == "" {
	...
	req.GroupInfo.NeedVerification = 2
...
func (s *groupServer) GetJoinedGroupList(ctx context.Context, req *pbgroup.GetJoinedGroupListReq) (*pbgroup.GetJoinedGroupListResp, error) {
	...
	total, groupIDs, err := s.db.PageGetJoinGroup(ctx, req.FromUserID, req.GroupIDs, req.Pagination)
	if err != nil {
		return nil, err
	}
	...
	//TODO 增加逻辑 从已加入的群搜索指定关键词的群名称
	if req.GroupName != "" {
		// 遍历切片并检查是否包含指定关键词
		var groupsTmp []*relationtb.GroupModel
		for _, item := range groups {
			if strings.Contains(strings.ToLower(item.GroupName), strings.ToLower(req.GroupName)) {
				groupsTmp = append(groupsTmp, item)
			}
		}
		resp.Total = uint32(len(groupsTmp))
		groups = groupsTmp
		groupsTmp = nil
	}
...
func (s *groupServer) InviteUserToGroup(ctx context.Context, req *pbgroup.InviteUserToGroupReq) (*pbgroup.InviteUserToGroupResp, error) {
	...
	var opUserID string
	var adminAuth = authverify.IsAppManagerUid(ctx)
	userAuth, _ := utils.VerifyRights(req.Ex)
	if !adminAuth && userAuth < 0 {
		opUserID = mcontext.GetOpUserID(ctx)
		var err error
	...
	// 获取当前会话 最大seq | 以设置当前用加群时的最小seq
	conversationId := "sg_" + req.GroupID
	maxSeq, _ := s.msgCache.GetMaxSeq(ctx, conversationId)
	maxSeq += 1
	var seqs = make(map[string]int64)
	var readSeqReqs msg2.MarkReadReq
		...
		if !adminAuth && userAuth < 0 {
			if !(groupMember.RoleLevel == constant.GroupOwner || groupMember.RoleLevel == constant.GroupAdmin) {
				...
				for _, userID := range req.InvitedUserIDs {
					...
					seqs[userID] = maxSeq
					var readSeqReq msg2.ReadSeqReq
					readSeqReq.Seq = maxSeq
					readSeqReq.ConversationID = conversationId
					readSeqReq.UserID = userID
					readSeqReqs.MarkReadReq = append(readSeqReqs.MarkReadReq, &readSeqReq)
				}
				if err := s.db.CreateGroupRequest(ctx, requests); err != nil {
					return nil, err
				}
				// 设置用户加群时最小Seq
				s.msgCache.SetConversationUserMinSeqs(ctx, conversationId, seqs)
				// 设置用户之前的消息的已读
				s.msgRpcClient.ReadSeqs(ctx, &readSeqReqs)
...
func (s *groupServer) GetGroupMemberList(ctx context.Context, req *pbgroup.GetGroupMemberListReq) (*pbgroup.GetGroupMemberListResp, error) {
	...
	if req.Keyword == "" {
		// 直接从mongodb获取
		if req.FromMongo == 1 {
			total, members, err = s.db.SearchGroupMemberSorted(ctx, "", req.GroupID, req.Pagination)
...
func (s *groupServer) GetGroupMemberPageList(ctx context.Context, req *pbgroup.GetGroupMemberListReq) (*pbgroup.GetGroupMemberListResp, error) {
	...
	members := make([]*relationtb.GroupMemberModel, 0)
	members, err = s.db.FindGroupMemberWaitGroup(ctx, req.GroupID)
	if err != nil {
		return nil, err
	}
	if req.Keyword == "" {
	...
	if err := s.PopulateGroupMemberWaitGroup(ctx, members...); err != nil {
		return nil, err
	}
	slices.SortStableFunc(members, func(a, b *relationtb.GroupMemberModel) int {
		if a.RoleLevel != b.RoleLevel {
			//降序
			return cmp.Compare(b.RoleLevel, a.RoleLevel)
		}
		if a.JoinTime.UnixMilli() != b.JoinTime.UnixMilli() {
			//升序
			return cmp.Compare(a.JoinTime.UnixMilli(), b.JoinTime.UnixMilli())
		}
		//升序
		return cmp.Compare(a.UserID, b.UserID)
	})
	friendsRemark := &friend.GetFriendsRemarkResp{}
	if loginUserId != "" && !authverify.CheckUserIDIsAdmin(loginUserId) {
		friendsRemark, _ = s.FriendRpcClient.GetFriendsRemark(ctx, loginUserId)
	}
	if req.Keyword != "" {
		groupMembers := make([]*relationtb.GroupMemberModel, 0)
		for _, member := range members {
			...
			if strings.Contains(strings.ToLower(member.Nickname), strings.ToLower(req.Keyword)) {
				groupMembers = append(groupMembers, member)
				total++
				continue
			}
			remark, exists := friendsRemark.FriendInfo[member.UserID]
			if exists && strings.Contains(strings.ToLower(remark), strings.ToLower(req.Keyword)) {
				groupMembers = append(groupMembers, member)
				total++
				continue
			}
		...
		GMembers := utils.Paginate(members, int(req.Pagination.GetPageNumber()), int(req.Pagination.GetShowNumber()))
		resp.Members = utils.Batch(convert.Db2PbGroupMember, GMembers)
	}
	resp.Total = uint32(total)
	if loginUserId != "" && !authverify.CheckUserIDIsAdmin(loginUserId) {
		for _, respMemberTmp := range resp.Members {
			remarkTmp, existsTmp := friendsRemark.FriendInfo[respMemberTmp.UserID]
			if existsTmp {
				respMemberTmp.Remark = remarkTmp
			}
		}
	}
	return resp, nil
...
func (s *groupServer) JoinGroup(ctx context.Context, req *pbgroup.JoinGroupReq) (resp *pbgroup.JoinGroupResp, err error) {
	...
	//TODO 新增逻辑，如果加入群的用户为 主播、运营、高权限账号，直接加入群
	auth, _ := utils.VerifyRights(req.Ex)
	...
	readSeqReq.Seq = maxSeq
	readSeqReq.ConversationID = conversationId
	readSeqReq.UserID = user.UserID
	readSeqReqs.MarkReadReq = append(readSeqReqs.MarkReadReq, &readSeqReq)
	if group.NeedVerification == constant.Directly || auth > 0 {
		groupMember := &relationtb.GroupMemberModel{
			GroupID:        group.GroupID,
			UserID:         user.UserID,
			RoleLevel:      constant.GroupOrdinaryUsers,
		...
		s.msgRpcClient.ReadSeqs(ctx, &readSeqReqs)
		// 设置用户加群时最小Seq
		s.msgCache.SetConversationUserMinSeq(ctx, conversationId, user.UserID, maxSeq)
		// 加群通知
...
func (s *groupServer) SetGroupInfo(ctx context.Context, req *pbgroup.SetGroupInfoReq) (*pbgroup.SetGroupInfoResp, error) {
	...
	if req.GroupInfoForSet.FaceURL != "" {
		num--
		_ = s.Notification.GroupInfoSetFaceURLNotification(ctx, &sdkws.GroupInfoSetFaceURLTips{Group: tips.Group, OpUser: tips.OpUser})
	}
	if num > 0 {
...
func (s *groupServer) GetGroups(ctx context.Context, req *pbgroup.GetGroupsReq) (*pbgroup.GetGroupsResp, error) {
	...
	if req.GroupID != "" {
		...
		var total int64
		total, group, err = s.db.SearchGroup(ctx, 0, "", req.GroupName, req.Pagination)
		resp.Total = uint32(total)
	}
	if err != nil {
		return nil, err
...
func (s *groupServer) GetFindGroups(ctx context.Context, req *pbgroup.GetFindGroupsReq) (*pbgroup.GetFindGroupsResp, error) {
	if err := authverify.CheckAccessV3(ctx, req.FromUserID); err != nil {
		return nil, err
	}
	resp := &pbgroup.GetFindGroupsResp{}
	...
	case 1:
		...
		//1.获取允许推荐且排除已加入的群
		var total int64
		total, group, err = s.db.SearchGroup(ctx, req.SearchType, req.FromUserID, "", req.Pagination)
		resp.Total = uint32(total)
		if err != nil {
			return nil, err
		}
...
// 消息页面搜索群昵称和群成员名称
func (s *groupServer) SearchGroupMembers(ctx context.Context, req *pbgroup.SearchGroupMembersReq) (*pbgroup.SearchGroupMembersResp, error) {
	if err := authverify.CheckAccessV3(ctx, req.FromUserID); err != nil {
		return nil, err
	}
	resp := &pbgroup.SearchGroupMembersResp{}
	var total int64
	total, group, err := s.db.SearchGroupMembers(ctx, req.GroupName, req.FromUserID, req.Pagination)
	if err != nil {
		return nil, err
	}
	resp.Total = uint32(total)
	var groups []*relationtb.SearchGroupMemberModel
	for _, v := range group {
		if v.Status == constant.GroupStatusDismissed {
			resp.Total--
			continue
	...
	groupIDs := utils.Slice(groups, func(e *relationtb.SearchGroupMemberModel) string {
		return e.GroupID
	})
	groupMemberNumMap, err := s.db.MapGroupMemberNum(ctx, groupIDs)
	if err != nil {
	...
	resp.Groups = utils.Slice(utils.Order(groupIDs, groups, func(group *relationtb.SearchGroupMemberModel) string {
		return group.GroupID
	}), func(group *relationtb.SearchGroupMemberModel) *sdkws.SearchGroupMemberInfo {
		return convert.Db2PbCMSSearchGroupMember(group, groupMemberNumMap[group.GroupID])
	})
	return resp, nil
}
