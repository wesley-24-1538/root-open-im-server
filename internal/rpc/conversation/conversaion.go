...
func (c *conversationServer) SetConversations(ctx context.Context,
	...
	if req.Conversation.IsPrivateChat != nil && req.Conversation.ConversationType != constant.SuperGroupChatType {
		...
		if err := c.conversationDatabase.SyncPeerUserPrivateConversationTx(ctx, conversations); err != nil {
			log.ZWarn(ctx, "Sync Peer User set Private notification failed", err, "conversations", conversations)
			return nil, err
		}
		wg := errgroup.Group{}
		limitLen := len(req.UserIDs)
		if limitLen > 200 {
			limitLen = 200
		}
		wg.SetLimit(limitLen)
		for _, userID := range req.UserIDs {
			userID := userID // safe closure var
			wg.Go(func() error {
	...
	if req.Conversation.LatestMsgSendTime != nil {
		m["latest_msg_send_time"] = req.Conversation.LatestMsgSendTime.Value
		if req.Conversation.LatestMsgSendTime.Value != conv.LatestMsgSendTime {
			unequal++
		}
	}
	...
	if unequal > 0 {
		...
		for _, v := range req.UserIDs {
			...
			wg.Go(func() error {
				err := c.conversationNotificationSender.ConversationChangeNotification(ctx, v, []string{req.Conversation.ConversationID})
				if err != nil {
					log.ZWarn(ctx, "send conversation set private notification failed", err,
						"userID", v, "conversationID", req.Conversation.ConversationID)
					return err
				}
				return nil
			})
