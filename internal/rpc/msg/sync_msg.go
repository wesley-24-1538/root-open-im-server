...
func (m *msgServer) PullMessageBySeqs(
	...
	userInfoTmp := &sdkws.UserInfo{}
	userInfoTmp, err := m.User.GetUserInfo(ctx, req.UserID)
	if userInfoTmp == nil {
		userInfo.UserID = req.UserID
		log.ZError(ctx, "PullMessageBySeqs GetUserInfo error", err, "req", req, "userInfo", userInfo, "userInfoTmp", userInfoTmp)
	} else {
		userInfo = userInfoTmp
	}
	friendsRemark := make(map[string]string)
	if userInfo.UserID != "" && !authverify.CheckUserIDIsAdmin(userInfo.UserID) {
		friendsRemarkResp, _ := m.friend.GetFriendsRemark(ctx, userInfo.UserID)
		friendsRemark = friendsRemarkResp.FriendInfo
	}
	for _, seq := range req.SeqRanges {
		if !msgprocessor.IsNotification(seq.ConversationID) {
			...
			if conversation.ConversationType == constant.SuperGroupChatType {
				members, err := m.GroupDatabase.TakeGroupMember(ctx, conversation.GroupID, req.UserID)
				if err == nil {
					memberCacheResp := pbgroup.GetGroupMemberCacheResp{Member: convert.Db2PbGroupMember(members)}
					groupMemberCache = memberCacheResp.Member
				}
			}
			minSeq, maxSeq, msgs, err := m.MsgDatabase.GetMsgBySeqsRange(ctx, userInfo, seq.ConversationID,
				seq.Begin, seq.End, seq.Num, conversation, groupMemberCache, friendsRemark)
			if err != nil {
				log.ZDebug(ctx, "GetMsgBySeqsRange error", "err", err, "conversationID", seq.ConversationID, "seq", seq, "req", req)
				continue
			}
			var isEnd bool
			switch req.Order {
			...
			if len(msgs) == 0 {
				log.ZWarn(ctx, "not have msgs", nil, "conversationID", seq.ConversationID, "seq", seq, "req", req)
				continue
			}
			...
			conversationArr := strings.Split(seq.ConversationID, "_")
			newConversationID := seq.ConversationID
			reg := regexp.MustCompile(`^n_`)
			if len(conversationArr) == 2 {
				newConversationID = reg.ReplaceAllString(newConversationID, "sg_")
			} else if len(conversationArr) == 3 {
				newConversationID = reg.ReplaceAllString(newConversationID, "si_")
			}
			conversation, err := m.Conversation.GetConversation(ctx, req.UserID, newConversationID)
			if err != nil {
				log.ZError(ctx, "GetConversation error", err, "conversationID", newConversationID, "seqs", seqs)
			}
			groupMemberCache := &sdkws.GroupMemberFullInfo{}
			if conversation.ConversationType == constant.SuperGroupChatType {
				members, err := m.GroupDatabase.TakeGroupMember(ctx, conversation.GroupID, req.UserID)
			...
			minSeq, maxSeq, notificationMsgs, err := m.MsgDatabase.GetMsgBySeqs(ctx, req.UserID, seq.ConversationID, seqs, userInfo, groupMemberCache)
			if err != nil {
