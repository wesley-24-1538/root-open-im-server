...
func (m *msgServer) GetConversationsHasReadAndMaxSeq(ctx context.Context, req *msg.GetConversationsHasReadAndMaxSeqReq) (resp *msg.GetConversationsHasReadAndMaxSeqResp, err error) {
	...
	if len(req.ConversationIDs) == 0 {
		...
		conversationIDs = utils2.Paginate(conversationIDs, int(req.Pagination.GetPageNumber()), int(req.Pagination.GetShowNumber()))
	} else {
...
func (m *msgServer) ReadSeqs(ctx context.Context, req *msg.MarkReadReq) (resp *msg.MarkConversationAsReadResp, err error) {
	for _, markReadReq := range req.MarkReadReq {
		userID := markReadReq.UserID
		conversationID := markReadReq.ConversationID
		msgSeq := markReadReq.Seq
		hasReadSeq, err := m.MsgDatabase.GetHasReadSeq(ctx, userID, conversationID)
		if err != nil && errs.Unwrap(err) != redis.Nil {
			log.ZError(ctx, "ReadSeq", err, "MarkReadReq", req)
			continue
		}
		var seqs []int64
		seqs = append(seqs, msgSeq)
		if msgSeq > hasReadSeq {
			err = m.MsgDatabase.SetHasReadSeq(ctx, userID, conversationID, msgSeq)
			if err != nil {
				log.ZError(ctx, "ReadSeq", err, "MarkReadReq", req)
				continue
			}
		...
		if err = m.sendMarkAsReadNotification(ctx, conversationID, constant.SingleChatType, userID,
			userID, seqs, hasReadSeq); err != nil {
			log.ZError(ctx, "ReadSeq", err, "MarkReadReq", req)
			continue
		}
	}
