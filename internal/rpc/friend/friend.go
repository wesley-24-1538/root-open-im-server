...
func (s *friendServer) GetFriendsInfo(ctx context.Context, req *pbfriend.GetSpecifiedFriendsInfoReq) (*pbfriend.GetSpecifiedFriendsInfoResp, error) {
	...
	friends, err := s.friendDatabase.FindFriends(ctx, req.OwnerUserID, req.UserIDList)
	friendMap := utils.SliceToMap(friends, func(e *tablerelation.FriendModel) string {
		return e.FriendUserID
	})
	resp := &pbfriend.GetSpecifiedFriendsInfoResp{
		Infos: make([]*pbfriend.GetSpecifiedFriendsInfoInfo, 0, len(friends)),
	}
...
func (s *friendServer) GetFriendPageList(ctx context.Context, req *pbfriend.GetPaginationFriendsReq) (resp *pbfriend.GetPaginationFriendsResp, err error) {
	defer log.ZInfo(ctx, utils.GetFuncName()+" Return")
	if err := s.userRpcClient.Access(ctx, req.UserID); err != nil {
		return nil, err
	}
	...
	if req.Keyword == "" {
		...
		//获取所有好友的用户信息
		friendsInfo, err := s.userRpcClient.GetUsersInfoWaitGroup(ctx, FriendIDs)
		if err != nil {
			return nil, err
		}
		//获取该登录用户所有好友的备注
		friendsRemark, _ := s.friendDatabase.FindFriendsRemark(ctx, req.UserID)
		friendUserInfo := make([]*sdkws.UserInfo, 0)
		var total int32
		for _, friendInfo := range friendsInfo {
			if friendInfo.UserID == req.Keyword {
				friendUserInfo = append(friendUserInfo, friendInfo)
				total++
				continue
			}
			if strings.Contains(strings.ToLower(friendInfo.Nickname), strings.ToLower(req.Keyword)) {
				friendUserInfo = append(friendUserInfo, friendInfo)
				total++
				continue
			}
			remark, exists := friendsRemark[friendInfo.UserID]
		...
		slices.SortStableFunc(friendUserInfo, func(a, b *sdkws.UserInfo) int {
			//升序
		...
		friendMembers := utils.Paginate(friendUserInfo, int(req.Pagination.GetPageNumber()), int(req.Pagination.GetShowNumber()))
		friendUserIDs := utils.Slice(friendMembers, func(e *sdkws.UserInfo) string {
			return e.UserID
		})
		friends, err := s.friendDatabase.FindFriendsWithError(ctx, req.UserID, friendUserIDs)
		friendsDB2Pb := make([]*sdkws.FriendInfo, 0)
		for _, friend := range friends {
			for _, info := range friendMembers {
				if info.UserID == friend.FriendUserID {
					tmp := &sdkws.FriendInfo{
						OwnerUserID: friend.OwnerUserID,
						Remark:      friend.Remark,
						CreateTime:  friend.CreateTime.Unix(),
					...
					friendsDB2Pb = append(friendsDB2Pb, tmp)
					break
				}
			}
		}
		resp.FriendsInfo = friendsDB2Pb
		resp.Total = total
	}
	return resp, nil
}
...
func (s *friendServer) GetFriendsRemark(ctx context.Context, req *pbfriend.GetFriendsRemarkReq) (*pbfriend.GetFriendsRemarkResp, error) {
	resp := &pbfriend.GetFriendsRemarkResp{}
	var friendsRemark map[string]string
	friendsRemark, err := s.friendDatabase.FindFriendsRemark(ctx, req.OwnerUserID)
	resp.FriendInfo = friendsRemark
	return resp, err
}
