...
# TODO: Label
openim::golang::server_targets() { ##wesley 这里可以去掉不启动的项目 ？如何传参进去
  local targets=(
    openim-api
    openim-cmdutils
    openim-crontask
  ...
  #如果有自定义启动项目，替换掉默认启动项目
  if [[  -n "${SERVER_TARGETS_FROM_CUSTOMER:-}" ]]; then
    targets=("${SERVER_TARGETS_FROM_CUSTOMER[@]}")
  fi
...
openim::golang::start_script_list() { ##wesley 这里可以去掉不启动的项目
  local targets=(
      openim-api.sh
      openim-rpc.sh
      openim-push.sh
  ...
  #如果有自定义启动项目，替换掉默认启动项目
  if [[ "${#EXECUTE_SCRIPTS_FROM_CUSTOMER[@]}" -ne 0 ]]; then
    targets=("${EXECUTE_SCRIPTS_FROM_CUSTOMER[@]}")
  fi
