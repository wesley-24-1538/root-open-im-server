...
openim::util::check_ports() {
  ...
  local ports=$(openim::util::list-to-string "$*")
...
openim::util::stop_services_on_ports() {
  ...
  local ports="$*"
  openim::log::info "Stopping services on ports: $ports"

  ports=$(openim::util::list-to-string "$ports")
...
function openim::util::split-space-to-comma() {
    local ports_list="$*"
    local ports_array=$(echo "$ports_list" | sed 's/,/ /g')
    echo "$ports_array"
}
