...
# 设置统一的密码，方便记忆
readonly PASSWORD=${PASSWORD:-'cR4YEHmJJtwU2PFym'}

# 设置统一的数据库名称，方便管理
def "DATABASE_NAME" "kqcc_openim"

# Linux系统 openim 用户
def "LINUX_USERNAME" "kqcc_openim"
...
# OpenIM Websocket端口
readonly OPENIM_WS_PORT=${OPENIM_WS_PORT:-'10001,10000,9999,9998'}
...
# Zookeeper的地址
readonly ZOOKEEPER_ADDRESS=${ZOOKEEPER_ADDRESS:-"mse-70b44b214-zk.mse.aliyuncs.com"}
...
def "MONGO_PORT" "3717"                       # MongoDB的端口
#def "MONGO_ADDRESS" "${DOCKER_BRIDGE_GATEWAY}" # MongoDB的地址
readonly MONGO_ADDRESS=${MONGO_ADDRESS:-"s-3nsa5c06968d7194.mongodb.rds.aliyuncs.com:3717,s-3ns832c4105c2b84.mongodb.rds.aliyuncs.com"}
def "MONGO_DATABASE" "${DATABASE_NAME}"        # MongoDB的数据库名
def "MONGO_USERNAME" "root"		               # MongoDB的管理员身份用户名
# MongoDB的管理员身份密码
readonly MONGO_PASSWORD=${MONGO_PASSWORD:-"cR4YEHmJJtwU2PFym"}
# Mongo OpenIM 身份用户名
def "MONGO_OPENIM_USERNAME" "kqcc_openim"
# Mongo OpenIM 身份密码
readonly MONGO_OPENIM_PASSWORD=${MONGO_OPENIM_PASSWORD:-"_cR4YEHmJJtwU2PFym_"}

def "MONGO_MAX_POOL_SIZE" "200"                # 最大连接池大小

###################### Object 配置信息 ######################
# app要能访问到此ip和端口或域名
readonly API_URL=${API_URL:-"https://openim-api.sigacc.com"}
...
# MinIO的端点URL
def MINIO_ADDRESS "192.168.6.14"
readonly MINIO_ENDPOINT=${MINIO_ENDPOINT:-"http://${MINIO_ADDRESS}:${MINIO_PORT}"}
def "MINIO_ACCESS_KEY" "openim"                                                  # MinIO的访问密钥ID
readonly MINIO_SECRET_KEY=${MINIO_SECRET_KEY:-"yfDm6CG3wQ5rjS2"}
def "MINIO_SESSION_TOKEN"                                                         # MinIO的会话令牌
readonly MINIO_SIGN_ENDPOINT=${MINIO_SIGN_ENDPOINT:-"https://oss.ylhgzs.com"} # signEndpoint为minio公网地址
...
###################### Redis 配置信息 ######################
def "REDIS_PORT" "6379"                                    # Redis的端口
# Redis的地址
def "REDIS_ADDRESS" "r-3nsx6dsxh8nq1m90ub.redis.rds.aliyuncs.com"
def "REDIS_USERNAME"                                        # Redis的用户名
readonly REDIS_PASSWORD=${REDIS_PASSWORD:-"Xpj7QeML86BpEKLbS"}
...
# `Kafka` 的地址
readonly KAFKA_ADDRESS=${KAFKA_ADDRESS:-"alikafka-post-public-intl-sg-6l13r019f0a-1-vpc.alikafka.aliyuncs.com:9092,alikafka-post-public-intl-sg-6l13r019f0a-2-vpc.alikafka.aliyuncs.com:9092,alikafka-post-public-intl-sg-6l13r019f0a-3-vpc.alikafka.aliyuncs.com"}
def "KAFKA_LATESTMSG_REDIS_TOPIC" "betaLatestMsgToRedis"        # `Kafka` 的最新消息到Redis的主题
def "KAFKA_OFFLINEMSG_MONGO_TOPIC" "betaOfflineMsgToMongoMysql" # `Kafka` 的离线消息到Mongo的主题
def "KAFKA_MSG_PUSH_TOPIC" "betaMsgToPush"                      # `Kafka` 的消息到推送的主题
def "KAFKA_CONSUMERGROUPID_REDIS" "betaRedis"                   # `Kafka` 的消费组ID到Redis
def "KAFKA_CONSUMERGROUPID_MONGO" "betaMongo"                   # `Kafka` 的消费组ID到Mongo
def "KAFKA_CONSUMERGROUPID_MYSQL" "betaMysql"                   # `Kafka` 的消费组ID到MySql
def "KAFKA_CONSUMERGROUPID_PUSH" "betaPush"                     # `Kafka` 的消费组ID到推送
def "KAFKA_TOPIC_PARTITION" "36"                            # `Kafka` 的topic分区数量
...
# OpenIM用户服务端口
readonly OPENIM_USER_PORT=${OPENIM_USER_PORT:-'10110,10111,10112,10113'}
# OpenIM朋友服务端口
readonly OPENIM_FRIEND_PORT=${OPENIM_FRIEND_PORT:-'10210,10211,10212,10213'}
# OpenIM消息服务端口
readonly OPENIM_MESSAGE_PORT=${OPENIM_MESSAGE_PORT:-'10310,10311,10312,10313'}
# OpenIM消息网关服务端口
readonly OPENIM_MESSAGE_GATEWAY_PORT=${OPENIM_MESSAGE_GATEWAY_PORT:-'10410,10411,10412,10413'}
# OpenIM组服务端口
readonly OPENIM_GROUP_PORT=${OPENIM_GROUP_PORT:-'10510,10511,10512,10513'}
# OpenIM授权服务端口
readonly OPENIM_AUTH_PORT=${OPENIM_AUTH_PORT:-'10610,10611,10612,10613'}
# OpenIM推送服务端口
readonly OPENIM_PUSH_PORT=${OPENIM_PUSH_PORT:-'10710,10711,10712,10713'}
# OpenIM对话服务端口
readonly OPENIM_CONVERSATION_PORT=${OPENIM_CONVERSATION_PORT:-'10810,10811,10812,10813'}
# OpenIM第三方服务端口
readonly OPENIM_THIRD_PORT=${OPENIM_THIRD_PORT:-'10910,10911,10912,10912'}
...
def "LOG_REMAIN_LOG_LEVEL" "3"                      # 保留的日志级别
...
def "MAX_CONN_WORKERS" "200"             # ws异步推送线程数
# GeTui推送URL
readonly GETUI_PUSH_URL=${GETUI_PUSH_URL:-'https://restapi.getui.com/v2/U4FhxTyt828hQer7nMNmh'}
def "GETUI_MASTER_SECRET" "0DNBJN4T6C7PQhIfsg0Rq1"          # GeTui主密钥
def "GETUI_APP_KEY" "Um9GJow07Z5Lzs0icohV81"                # GeTui应用密钥
def "GETUI_INTENT" "startapp"                 # GeTui推送意图
def "GETUI_CHANNEL_ID" "appStore"             # GeTui渠道ID
def "GETUI_CHANNEL_NAME" "人人体育"           # GeTui渠道名称
...
def "NICKNAME_1" "系统管理员"            # 昵称1
def "NICKNAME_2" "系统管理员"            # 昵称2
def "NICKNAME_3" "系统管理员"            # 昵称3
def "IM_ADMIN_USERID" "imAdmin"       # IM管理员ID
def "IM_ADMIN_NAME" "系统管理员"         # IM管理员昵称
def "MULTILOGIN_POLICY" "0"           # 多登录策略
...
readonly USER_PROM_PORT=${USER_PROM_PORT:-'20110,20111,20112,20113'}
# Friend 服务的 Prometheus 端口
readonly FRIEND_PROM_PORT=${FRIEND_PROM_PORT:-'20210,20211,20212,20213'}
# Message 服务的 Prometheus 端口
readonly MESSAGE_PROM_PORT=${MESSAGE_PROM_PORT:-'20310,20311,20312,20313'}
# Message Gateway 服务的 Prometheus 端口
readonly MSG_GATEWAY_PROM_PORT=${MSG_GATEWAY_PROM_PORT:-'20410,20411,20412,20413'}
# Group 服务的 Prometheus 端口
readonly GROUP_PROM_PORT=${GROUP_PROM_PORT:-'20510,20511,20512,20513'}
# Auth 服务的 Prometheus 端口
readonly AUTH_PROM_PORT=${AUTH_PROM_PORT:-'20610,20611,20612,20613'}
# Push 服务的 Prometheus 端口
readonly PUSH_PROM_PORT=${PUSH_PROM_PORT:-'20710,20711,20712,20713'}
# Conversation 服务的 Prometheus 端口
readonly CONVERSATION_PROM_PORT=${CONVERSATION_PROM_PORT:-'20810,20811,20812,20813'}
# Third 服务的 Prometheus 端口
readonly THIRD_PROM_PORT=${THIRD_PROM_PORT:-'20910,20911,20912,20913'}
