...
IFS=" " read -ra OPENIM_RPC_TO_PROMETHEUS_PORT_TARGETS <<< "$(openim::rpc::prometheus_port)"
readonly OPENIM_RPC_TO_PROMETHEUS_PORT_TARGETS
readonly OPENIM_RPC_PROMETHEUS_PORT_LISTARIES=("${OPENIM_RPC_TO_PROMETHEUS_PORT_TARGETS[@]##*/}")
...
function openim::rpc::start() {
    ...
    echo "OPENIM_RPC_PROMETHEUS_PORT_LISTARIES: ${OPENIM_RPC_PROMETHEUS_PORT_LISTARIES[@]}"
    ...
    printf "| %-22s | %-25s | %-25s |\n" "${OPENIM_RPC_SERVICE_LISTARIES[$i]}" "${OPENIM_RPC_PORT_LISTARIES[$i]}" "${OPENIM_RPC_PROMETHEUS_PORT_LISTARIES[$i]}"
    printf "+------------------------+----------------------------+--------------------------+\n"
    done
    ...
    for ((i = 0; i < ${#OPENIM_RPC_SERVICE_LISTARIES[*]}; i++)); do
        ...
        OPENIM_RPC_PROMETHEUS_PORTS=( $(openim::util::list-to-string ${OPENIM_RPC_PROMETHEUS_PORT_LISTARIES[$i]}) )
        read -a OPENIM_RPC_PROM_PORTS_ARRAY <<< ${OPENIM_RPC_PROMETHEUS_PORTS}

        for ((j = 0; j < ${#OPENIM_RPC_SERVICE_PORTS[@]}; j++)); do
            openim::log::info "Starting ${OPENIM_RPC_SERVICE_LISTARIES[$i]} ${OPENIM_RPC_SERVICE_PORTS[$j]}->${OPENIM_RPC_PROMETHEUS_PORTS[$j]}"
            openim::rpc::start_service "${OPENIM_RPC_SERVICE_LISTARIES[$i]}" "${OPENIM_RPC_SERVICE_PORTS[$j]}" "${OPENIM_RPC_PROMETHEUS_PORTS[$j]}"
    ...
    # openim::util::check_ports ${OPENIM_RPC_TO_PROMETHEUS_PORT_TARGETS[@]}
