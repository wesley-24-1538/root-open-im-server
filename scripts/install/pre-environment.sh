...
# 设置统一的数据库名称，方便管理
def "DATABASE_NAME" "pre_openim_v3"
...
# Zookeeper的地址
readonly ZOOKEEPER_ADDRESS=${ZOOKEEPER_ADDRESS:-"mse-3d1b67e14-zk.mse.aliyuncs.com"}
...
readonly MONGO_ADDRESS=${MONGO_ADDRESS:-"s-j6cae9ae0d06ac34.mongodb.rds.aliyuncs.com:3717,s-j6cab9c25e9fef64.mongodb.rds.aliyuncs.com"}
def "MONGO_DATABASE" "${DATABASE_NAME}"        # MongoDB的数据库名
def "MONGO_USERNAME" "root"		               # MongoDB的管理员身份用户名
# MongoDB的管理员身份密码
readonly MONGO_PASSWORD=${MONGO_PASSWORD:-"__WC68raKAKY2yzmxjmN__"}
# Mongo OpenIM 身份用户名
def "MONGO_OPENIM_USERNAME" "pre_openIM"
# Mongo OpenIM 身份密码
readonly MONGO_OPENIM_PASSWORD=${MONGO_OPENIM_PASSWORD:-"_openIM123_"}

def "MONGO_MAX_POOL_SIZE" "500"                # 最大连接池大小

###################### Object 配置信息 ######################
# app要能访问到此ip和端口或域名
readonly API_URL=${API_URL:-"https://pre-openim-api.88zb3355.com"}
...
# MinIO的端点URL
def MINIO_ADDRESS "172.31.13.63"
readonly MINIO_ENDPOINT=${MINIO_ENDPOINT:-"http://${MINIO_ADDRESS}:${MINIO_PORT}"}
def "MINIO_ACCESS_KEY" "openim"                                                  # MinIO的访问密钥ID
readonly MINIO_SECRET_KEY=${MINIO_SECRET_KEY:-"brooks12580"}
def "MINIO_SESSION_TOKEN"                                                         # MinIO的会话令牌
readonly MINIO_SIGN_ENDPOINT=${MINIO_SIGN_ENDPOINT:-"https://pre-oss.88zb3355.com"} # signEndpoint为minio公网地址
...
# Redis的地址
def "REDIS_ADDRESS" "r-j6choiv7bo6pvmtpsq.redis.rds.aliyuncs.com"
def "REDIS_USERNAME"                                        # Redis的用户名
readonly REDIS_PASSWORD=${REDIS_PASSWORD:-"xEDvMXR4PTG"}
...
# `Kafka` 的地址
readonly KAFKA_ADDRESS=${KAFKA_ADDRESS:-"alikafka-post-cn-5yd3l6va9008-1-vpc.alikafka.aliyuncs.com:9092,alikafka-post-cn-5yd3l6va9008-2-vpc.alikafka.aliyuncs.com:9092,alikafka-post-cn-5yd3l6va9008-3-vpc.alikafka.aliyuncs.com"}
def "KAFKA_LATESTMSG_REDIS_TOPIC" "preLatestMsgToRedis"        # `Kafka` 的最新消息到Redis的主题
def "KAFKA_OFFLINEMSG_MONGO_TOPIC" "preOfflineMsgToMongoMysql" # `Kafka` 的离线消息到Mongo的主题
def "KAFKA_MSG_PUSH_TOPIC" "preMsgToPush"                      # `Kafka` 的消息到推送的主题
def "KAFKA_CONSUMERGROUPID_REDIS" "preRedis"                   # `Kafka` 的消费组ID到Redis
def "KAFKA_CONSUMERGROUPID_MONGO" "preMongo"                   # `Kafka` 的消费组ID到Mongo
def "KAFKA_CONSUMERGROUPID_MYSQL" "preMysql"                   # `Kafka` 的消费组ID到MySql
def "KAFKA_CONSUMERGROUPID_PUSH" "prePush"                     # `Kafka` 的消费组ID到推送
def "KAFKA_TOPIC_PARTITION" "24"                            # `Kafka` 的topic分区数量
...
# GeTui推送URL
readonly GETUI_PUSH_URL=${GETUI_PUSH_URL:-'https://restapi.getui.com/v2/RU2D4vP8377pKt4Ufbutd5'}
def "GETUI_MASTER_SECRET" "uppj9oqxfn6QiaYMPgFna5"          # GeTui主密钥
def "GETUI_APP_KEY" "L8TeZkHMPl6e8my6pDwrh1"                # GeTui应用密钥
