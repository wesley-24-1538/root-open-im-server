...
function openim::msggateway::start() {
  ...
  OPENIM_MESSAGE_GATEWAY_PORTS=$(openim::util::list-to-string ${OPENIM_MESSAGE_GATEWAY_PORT} )
    ...
    local wsLength=${#OPENIM_WS_PORTS_ARRAY[@]}
    ...
    local msgLength=${#MSG_GATEWAY_PROM_PORTS_ARRAY[@]}
    ...
    openim::log::info "OPENIM_WS_PORTS_ARRAY:$OPENIM_WS_PORTS"
    openim::log::info "MSG_GATEWAY_PROM_PORTS:$MSG_GATEWAY_PROM_PORTS"
    openim::log::info "wsLength->msgLength:$wsLength->$msgLength"
    if [[ wsLength -ne msgLength ]]; then
