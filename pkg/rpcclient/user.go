...
// GetUsersInfoWaitGroup retrieves information for multiple users based on their user IDs.
func (u *UserRpcClient) GetUsersInfoWaitGroup(ctx context.Context, userIDs []string) ([]*sdkws.UserInfo, error) {
	if len(userIDs) == 0 {
		return []*sdkws.UserInfo{}, nil
	}
	resp, err := u.Client.GetDesignateUsersWaitGroup(ctx, &user.GetDesignateUsersReq{
		UserIDs: userIDs,
	})
	if err != nil {
		return nil, err
