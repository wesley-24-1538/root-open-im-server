...
func newContentTypeConf() map[int32]config.NotificationConf {
	log.ZInfo(context.Background(), "config-load", "notification", config.Config.Notification)
	return map[int32]config.NotificationConf{
		...
		constant.GroupInfoSetFaceURLNotification:          {IsSendMsg: true, ReliabilityLevel: constant.ReliableNotificationNoMsg},
		// user
...
func newSessionTypeConf() map[int32]int32 {
	return map[int32]int32{
		...
		constant.GroupInfoSetFaceURLNotification:          constant.SuperGroupChatType,
		// user
...
func (m *MessageRpcClient) GetMinSeqs(ctx context.Context, conversationIDs []string) (map[string]int64, error) {
	log.ZDebug(ctx, "GetMinSeqs", "conversationIDs", conversationIDs)
	resp, err := m.Client.GetMinSeqs(ctx, &msg.GetMinSeqsReq{
		ConversationIDs: conversationIDs,
	})
	return resp.MinSeqs, err
}
...
func (m *MessageRpcClient) ReadSeqs(ctx context.Context, req *msg.MarkReadReq) error {
	_, err := m.Client.ReadSeqs(ctx, req)
	return err
}
...
func (s *NotificationSender) NotificationWithSessionType(ctx context.Context, sendID, recvID string, contentType, sesstionType int32, m proto.Message, opts ...NotificationOptions) (err error) {
	...
	if err != nil {
		log.ZError(ctx, "MsgClient Notification json.Marshal failed", err, "sendID", sendID, "recvID", recvID, "contentType", contentType, "msgData", m)
		return err
	}
	...
	if notificationOpt.WithRpcGetUsername && s.getUserInfo != nil {
		...
		if err != nil {
			...
			msgData.SenderNickname = userInfo.Nickname
			msgData.SenderFaceURL = userInfo.FaceURL
		}
	}
	...
	msgData.MsgFrom = constant.SysMsgType
	msgData.ContentType = contentType
	msgData.SessionType = sesstionType
	if msgData.SessionType == constant.SuperGroupChatType {
		msgData.GroupID = recvID
	}
	...
	offlineInfo.Ex = ex
	msgData.OfflinePushInfo = &offlineInfo
	req.MsgData = &msgData
