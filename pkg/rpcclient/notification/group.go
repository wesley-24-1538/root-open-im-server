...
func NewGroupNotificationSender(
	...
	fnWaitGroup func(ctx context.Context, userIDs []string) ([]CommonUser, error),
	...
	return &GroupNotificationSender{
		...
		getUsersInfo:          fn,
		getUsersInfoWaitGroup: fnWaitGroup,
		db:                    db,
	}
}
...
type GroupNotificationSender struct {
	*rpcclient.NotificationSender
	...
	getUsersInfoWaitGroup func(ctx context.Context, userIDs []string) ([]CommonUser, error)
	db                    controller.GroupDatabase
}
...
func (g *GroupNotificationSender) PopulateGroupMemberWaitGroup(ctx context.Context, members ...*relation.GroupMemberModel) error {
	if len(members) == 0 {
		return nil
	}
	emptyUserIDs := make(map[string]struct{})
	...
	if len(emptyUserIDs) > 0 {
		users, err := g.getUsersInfoWaitGroup(ctx, utils.Keys(emptyUserIDs))
		if err != nil {
			return err
		}
		userMap := make(map[string]CommonUser)
...
func (g *GroupNotificationSender) GroupInfoSetFaceURLNotification(ctx context.Context, tips *sdkws.GroupInfoSetFaceURLTips) (err error) {
	defer log.ZDebug(ctx, "return")
	defer func() {
		if err != nil {
			log.ZError(ctx, utils.GetFuncName(1)+" failed", err)
	...
	return g.Notification(ctx, mcontext.GetOpUserID(ctx), tips.Group.GroupID, constant.GroupInfoSetFaceURLNotification, tips)
}
