...
func batchGetCacheWaitGroup[T any, K comparable](
	ctx context.Context,
	rcClient *rockscache.Client,
	expire time.Duration,
	keys []K,
	...
	wg := errgroup.Group{}
	var mu sync.Mutex
	wg.SetLimit(utils.WaitGroupSetLimit(length))
	for _, key := range keys {
		tmpKey := key // safe closure var
		wg.Go(func() error {
			val, err := getCache(ctx, rcClient, keyFn(tmpKey), expire, func(ctx context.Context) (T, error) {
				return fns(ctx, tmpKey)
			})
			if err != nil {
				return err
			}
