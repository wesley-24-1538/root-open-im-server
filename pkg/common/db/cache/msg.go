...
const (
	...
	revokeMsgConversationId = "REVOKE_CONVERSATION_ID:"

	msgToEsMQKey       = "live_admin:es:msg"  // 消息入es消费
	revokeMsgToEsMQKey = "openIm:revoke:list" // 撤回消息入es消费
)
...
type MsgModel interface {
	SeqCache
	thirdCache
	GetReds() redis.UniversalClient // 获取Redis实例
	MsgToEsMQ(ctx context.Context, conversationID string, msgs []*sdkws.MsgData) error
	...
	SetPushNotificationResultToRedis(ctx context.Context, clientMsgID string) error
...
// GetReds 获取Redis实例
func (c *msgCache) GetReds() redis.UniversalClient {
	return c.rdb
}

// MsgToEsMQ 消息推送至ES队列
func (c *msgCache) MsgToEsMQ(ctx context.Context, conversationID string, msgs []*sdkws.MsgData) error {
	var esMq = make(map[string]interface{})
	esMq["conversationId"] = conversationID
	esMq["msgs"] = msgs
	byteMsg, _ := json.Marshal(esMq)
	return c.rdb.LPush(ctx, msgToEsMQKey, string(byteMsg)).Err()
}

// RevokeMsgToEsMQ 撤回消息推送至ES队列
func (c *msgCache) RevokeMsgToEsMQ(ctx context.Context, _id string) error {
	return c.rdb.LPush(ctx, revokeMsgToEsMQKey, _id).Err()
}
...
func (c *msgCache) getRevokeConversationKey(clientId string) string {
	return revokeMsgConversationId + clientId
}
...
func (c *msgCache) PipeGetMessagesBySeq(ctx context.Context, conversationID string, seqs []int64) (seqMsgs []*sdkws.MsgData, failedSeqs []int64, err error) {
	...
	results := make([]*redis.StringCmd, 0, len(seqs))
...
func (c *msgCache) ParallelGetMessagesBySeq(ctx context.Context, conversationID string, seqs []int64) (seqMsgs []*sdkws.MsgData, failedSeqs []int64, err error) {
	...
	wg := errgroup.Group{}
	seqLength := len(seqs)
	wg.SetLimit(utils.WaitGroupSetLimit(seqLength))
	...
	for idx, seq := range seqs {
		...
		wg.Go(func() error {
			...
			if err != nil {
				log.ZDebug(ctx, "GetMessagesBySeq failed", err, "conversationID", conversationID, "seq", seq)
...
func (c *msgCache) ParallelSetMessageToCache(ctx context.Context, conversationID string, msgs []*sdkws.MsgData) (int, error) {
	wg := errgroup.Group{}
	msgLength := len(msgs)
	wg.SetLimit(utils.WaitGroupSetLimit(msgLength))
	for _, msg := range msgs {
...
func (c *msgCache) SetPushNotificationResultToRedis(ctx context.Context, clientMsgID string) error {
	operationID := mcontext.GetOperationID(ctx)
	parts := strings.Split(operationID, "$")
	redisKey := "PUSH_NOTIFICATION_RESULT:" + operationID
	if len(parts) > 1 {
		redisKey = "PUSH_NOTIFICATION_RESULT:" + parts[0]
	}
	c.rdb.Set(ctx, redisKey, clientMsgID, 10*time.Second)
	return nil
}

func (c *msgCache) SetRevokeConversationIdExpire(ctx context.Context, conversationID, clientMsgID string) error {
	c.rdb.Set(ctx, c.getRevokeConversationKey(clientMsgID), conversationID, 86400*time.Second)
	return nil
}

func (c *msgCache) GetRevokeConversationIdExpire(ctx context.Context, clientMsgID string) (string, error) {
	conversationID, err := c.rdb.Get(ctx, c.getRevokeConversationKey(clientMsgID)).Result()
