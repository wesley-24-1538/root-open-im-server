...
func (g *GroupMgo) Search(ctx context.Context, keyword string, pagination pagination.Pagination, SearchType int32, groupIDS []string) (total int64, groups []*relation.GroupModel, err error) {
	switch SearchType {
	case 1:
		//发现群-获取允许推荐且排除已加入的群
		return mgoutil.FindPage[*relation.GroupModel](ctx, g.coll, bson.M{"need_verification": 2, "group_id": bson.M{"$nin": groupIDS}}, pagination)
	default:
		//原逻辑
		return mgoutil.FindPage[*relation.GroupModel](ctx, g.coll, bson.M{"group_name": bson.M{"$regex": regexp.QuoteMeta(keyword), "$options": "i"}}, pagination)
	}
}
