...
type GroupDatabase interface {
	...
	SearchGroup(ctx context.Context, SearchType int32, userID string, keyword string, pagination pagination.Pagination) (int64, []*relationtb.GroupModel, error)
	...
	FindGroupMemberWaitGroup(ctx context.Context, groupID string) (groupMembers []*relationtb.GroupMemberModel, err error)                      // *
	...
	PageGetJoinGroup(ctx context.Context, userID string, searchGroupIDs []string, pagination pagination.Pagination) (total int64, groupIds []string, err error)
	...
	SearchGroupMemberSorted(ctx context.Context, keyword string, groupID string, pagination pagination.Pagination) (int64, []*relationtb.GroupMemberModel, error)
	...
	// CountRangeEverydayTotal 获取范围内群增量
	CountRangeEverydayTotal(ctx context.Context, start time.Time, end time.Time) (map[string]int64, error)
	DeleteGroupMemberHash(ctx context.Context, groupIDs []string) error
	// SearchGroupMembers 消息主页面搜索群昵称和群成员昵称
	SearchGroupMembers(ctx context.Context, keyword string, userID string, pagination pagination.Pagination) (int64, []*relationtb.SearchGroupMemberModel, error)
}
...
func (g *groupDatabase) FindGroupMemberWaitGroup(ctx context.Context, groupID string) ([]*relationtb.GroupMemberModel, error) {
	return g.cache.GetAllGroupMembersInfoWaitGroup(ctx, groupID)
}
...
func (g *groupDatabase) SearchGroup(ctx context.Context, SearchType int32, userID string, keyword string, pagination pagination.Pagination) (int64, []*relationtb.GroupModel, error) {
	switch SearchType {
	case 1:
		//发现群
		//1.获取已加入的群ID
		...
		//2.获取允许推荐且排除已加入的群
		return g.groupDB.Search(ctx, keyword, pagination, SearchType, groupIDs)
	default:
		//原搜索逻辑
		return g.groupDB.Search(ctx, keyword, pagination, 0, nil)
	}
}
...
func (g *groupDatabase) PageGetJoinGroup(ctx context.Context, userID string, searchGroupIDs []string, pagination pagination.Pagination) (total int64, totalGroupMembers []string, err error) {
	groupIDs, err := g.cache.GetJoinedGroupIDs(ctx, userID)
	if err != nil {
		return 0, nil, err
	}
	//增加逻辑 判断是否加入指定群
	if searchGroupIDs != nil {
		groupIDsTmp := make([]string, 0)
		for _, searchGroupID := range searchGroupIDs {
			if utils.InArray(searchGroupID, groupIDs) {
				groupIDsTmp = append(groupIDsTmp, searchGroupID)
			}
		}
		groupIDs = groupIDsTmp
	}
	groupIDsTmp := utils.Paginate(groupIDs, int(pagination.GetPageNumber()), int(pagination.GetShowNumber()))
	...
	//}
	return int64(len(groupIDs)), groupIDsTmp, nil
}
...
func (g *groupDatabase) SearchGroupMemberSorted(ctx context.Context, keyword string, groupID string, pagination pagination.Pagination) (int64,
	[]*relationtb.GroupMemberModel, error) {
	return g.groupMemberDB.SearchMemberSorted(ctx, keyword, groupID, pagination)
}
...
func (g *groupDatabase) SearchGroupMembers(ctx context.Context, keyword string, userID string, pagination pagination.Pagination) (int64, []*relationtb.SearchGroupMemberModel, error) {
	groupIDs, err := g.cache.GetJoinedGroupIDs(ctx, userID)
	if err != nil {
		return 0, nil, err
	}
	return g.groupMemberDB.SearchGroupMembers(ctx, keyword, userID, groupIDs, pagination)
}
