...
type UserDatabase interface {
	...
	FindWithErrorWaitGroup(ctx context.Context, userIDs []string) (users []*relation.UserModel, err error)
...
// FindWithErrorWaitGroup Get the information of the specified user and return an error if the userID is not found.
func (u *userDatabase) FindWithErrorWaitGroup(ctx context.Context, userIDs []string) (users []*relation.UserModel, err error) {
	users, err = u.cache.GetUsersInfoWaitGroup(ctx, userIDs)
	if err != nil {
		return
	}
	if len(users) != len(userIDs) {
