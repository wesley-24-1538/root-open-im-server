...
// 每次拉取消息数
const pageMsgLimit = 50

// 每次从mongoDB获取消息数
const pageMsgSize = 80
...
type CommonMsgDatabase interface {
	...
	GetMsgBySeqsRange(ctx context.Context, userInfo *sdkws.UserInfo, conversationID string, begin, end, num int64,
		conversation *conversation.Conversation, groupMemberCache *sdkws.GroupMemberFullInfo, friendsRemark map[string]string) (
		minSeq int64, maxSeq int64, seqMsg []*sdkws.MsgData, err error)

	// 通过seqList获取大群在 mongo里面的消息
	GetMsgBySeqs(ctx context.Context, userID string, conversationID string, seqs []int64, userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (minSeq int64, maxSeq int64, seqMsg []*sdkws.MsgData, err error)
	...
	GetRedis() redis.UniversalClient // 获取Redis实例
	MsgToEsMQ(ctx context.Context, conversationID string, esMsg []*sdkws.MsgData) error
	RevokeMsgToEsMQ(ctx context.Context, _id string) error
	// to mq
	...
	// MsgIdGetConversations 根据消息ID获取会话ID
	MsgIdGetConversations(ctx context.Context, req *pbmsg.MsgIdGetConversationsReq) (conversationIDs map[string]string, err error)
	// MsgIdGetConversationSeq 根据消息ID获取会话ID和当前的Seq
	MsgIdGetConversationSeq(ctx context.Context, req *pbmsg.MsgIdGetConversationSeqReq) (resp *pbmsg.MsgIdGetConversationSeqResp, err error)
	SetPushNotificationResultToRedis(ctx context.Context, clientMsgID string) (err error)
	SetRevokeConversationIdExpire(ctx context.Context, conversationID, clientMsgID string) error
	GetRevokeConversationIdExpire(ctx context.Context, clientMsgID string) (string, error)
}
...
func (db *commonMsgDatabase) SetRevokeConversationIdExpire(ctx context.Context, conversationID, clientMsgID string) error {
	return db.cache.SetRevokeConversationIdExpire(ctx, conversationID, clientMsgID)
}

func (db *commonMsgDatabase) GetRevokeConversationIdExpire(ctx context.Context, clientMsgID string) (string, error) {
	return db.cache.GetRevokeConversationIdExpire(ctx, clientMsgID)
}

func (db *commonMsgDatabase) SetPushNotificationResultToRedis(ctx context.Context, clientMsgID string) (err error) {
	return db.cache.SetPushNotificationResultToRedis(ctx, clientMsgID)
}

func (db *commonMsgDatabase) GetRedis() redis.UniversalClient {
	return db.cache.GetReds()
}

func (db *commonMsgDatabase) MsgToEsMQ(ctx context.Context, conversationID string, esMsg []*sdkws.MsgData) error {
	return db.cache.MsgToEsMQ(ctx, conversationID, esMsg)
}

func (db *commonMsgDatabase) RevokeMsgToEsMQ(ctx context.Context, _id string) error {
	return db.cache.RevokeMsgToEsMQ(ctx, _id)
}
...
func (db *commonMsgDatabase) getMsgBySeqs(ctx context.Context, userID, conversationID string, seqs []int64,
	userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (totalMsgs []*sdkws.MsgData, err error) {
	for docID, seqs := range db.msg.GetDocIDSeqsMap(conversationID, seqs) {
		msgs, err := db.findMsgInfoBySeq(ctx, userID, docID, conversationID, seqs, userInfo, groupMemberCache)
		if err != nil {
			return nil, err
		}
...
func (db *commonMsgDatabase) findMsgInfoBySeq(ctx context.Context, userID, docID string, conversationID string, seqs []int64, userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (totalMsgs []*unrelationtb.MsgInfoModel, err error) {
	...
	// 获取当前用户的拓展字段和当前用户所在群的权限
	userInfoEx := userInfo.Ex
	userAuth, _ := utils.VerifyRights(userInfoEx)
	var roleLevel int32 = 20
	if groupMemberCache != nil {
		roleLevel = groupMemberCache.RoleLevel
	}
	totalMsgs = make([]*unrelationtb.MsgInfoModel, 0)
	for _, msg := range msgs {
		//先处理业务逻辑，在过滤
		db.handlerDBMsg(ctx, tempCache, userID, conversationID, msg)
		if utils.FilterMsg(userAuth, roleLevel, msg, userInfo) {
			totalMsgs = append(totalMsgs, msg)
			if len(totalMsgs) >= pageMsgLimit {
				//最终只返回50条
				break
			}
		}
	}
	return totalMsgs, err
}

func (db *commonMsgDatabase) findMsgInfoByConversationID(ctx context.Context, userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo, conversationID string, groupID string,
	conversationType int32, startSeq, endSeq, num int64) (seqMsgs []*sdkws.MsgData, err error) {
	conversationTempId := conversationID
	if conversationType == constant.SuperGroupChatType {
		conversationTempId = groupID
	}
	totalMsgs := make([]*unrelationtb.MsgInfoModel, 0)
	// 获取当前用户的拓展字段和当前用户所在群的权限
	userInfoEx := userInfo.Ex
	...
	msgs, err := db.msgDocDatabase.GetMsgByConversationId(ctx, userInfo.UserID, conversationTempId, conversationType, startSeq, endSeq, userAuth, roleLevel)
	if err != nil {
		return nil, err
	}
	tempCache := make(map[int64][]*unrelationtb.MsgInfoModel)
	for _, msg := range msgs {
		totalLen := len(totalMsgs)
		endSeq = msg.Msg.Seq
		if utils.FilterMsg(userAuth, roleLevel, msg, userInfo) && int64(totalLen) < num {
			db.handlerDBMsg(ctx, tempCache, userInfo.UserID, conversationID, msg)
			totalMsgs = append(totalMsgs, msg)
		} else if int64(totalLen) >= num {
			break
		}
	}
	seqMsgs = make([]*sdkws.MsgData, 0)
	for _, sdkMsg := range totalMsgs {
		if sdkMsg.IsRead {
			sdkMsg.Msg.IsRead = true
		}
		seqMsgs = append(seqMsgs, convert.MsgDB2Pb(sdkMsg.Msg))
	}
	return seqMsgs, err
}

func (db *commonMsgDatabase) getMsgBySeqsRange(ctx context.Context, userID string, conversationID string, allSeqs []int64, minSeq, end int64,
	userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (seqMsgs []*sdkws.MsgData, err error) {
	log.ZDebug(ctx, "getMsgBySeqsRange", "conversationID", conversationID, "allSeqs", allSeqs, "begin", minSeq, "end", end)
...
//	userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (seqMsgs []*sdkws.MsgData, err error) {
//	log.ZDebug(ctx, "getMsgBySeqsRange", "conversationID", conversationID, "allSeqs", allSeqs, "begin", minSeq, "end", end)
//	seqMsgs, err = db.getFilterMsg(ctx, userID, conversationID, allSeqs, userInfo, groupMemberCache)
//	if err != nil {
//		return nil, err
//	}
//	if len(seqMsgs) < pageMsgLimit && end > minSeq {
//		// end 上次获取最小的最末尾消息的seq
//		// minSeq 检索消息最小Seq上限
//		var breakStatus bool
//		for {
//			tmpSeqs := utils.GetMsgSeqs(minSeq, end, pageMsgSize)
//			if len(tmpSeqs) == 0 {
//				//没有获取到消息组的Seq
//				break
//			}
//			tmpSeqMsgs, err := db.getFilterMsg(ctx, userID, conversationID, tmpSeqs, userInfo, groupMemberCache)
//			if err != nil {
//				return seqMsgs, err
//			}
//			for _, dataMsg := range tmpSeqMsgs {
//				seqMsgs = append(seqMsgs, dataMsg)
//				if len(seqMsgs) >= pageMsgLimit {
//					//最终只返回50条
//					breakStatus = true
//					break
//				}
...
//				break
//			}
//			end = tmpSeqs[len(tmpSeqs)-1]
//		}
//	}
//	return seqMsgs, nil
//}

func (db *commonMsgDatabase) getFilterMsg(ctx context.Context, userID, conversationID string, allSeqs []int64, userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (seqMsgs []*sdkws.MsgData, err error) {
	var breakStatus bool
	for docID, seqs := range db.msg.GetDocIDSeqsMap(conversationID, allSeqs) {
		msgs, err := db.findMsgInfoBySeq(ctx, userID, docID, conversationID, seqs, userInfo, groupMemberCache)
		if err != nil {
...
func (db *commonMsgDatabase) GetMsgBySeqsRange(ctx context.Context, userInfo *sdkws.UserInfo, conversationID string, begin, end, num int64,
	conversation *conversation.Conversation, groupMemberCache *sdkws.GroupMemberFullInfo, friendsRemark map[string]string) (
	int64, int64, []*sdkws.MsgData, error) {
	userMaxSeq := conversation.MaxSeq
	userMinSeq, err := db.cache.GetConversationUserMinSeq(ctx, conversationID, userInfo.UserID)
	if err != nil && errs.Unwrap(err) != redis.Nil {
		return 0, 0, nil, err
	}
	...
	if end < begin {
		log.ZDebug(ctx, "GetMsgBySeqsRange", "userMinSeq", userMinSeq, "conMinSeq", minSeq, "conMaxSeq", maxSeq, "userMaxSeq", userMaxSeq, "begin", begin, "end", end)
	...
	if len(seqs) == 0 {
		log.ZWarn(ctx, "GetMsgBySeqsRange", nil, "userMinSeq", userMinSeq, "conMinSeq", minSeq, "conMaxSeq", maxSeq, "userMaxSeq", userMaxSeq, "begin", begin, "end", end)
		return 0, 0, nil, nil
	}
	newBegin := seqs[0]
	...
	if len(cachedMsgs) > 0 {
		delSeqs, err := db.cache.GetUserDelList(ctx, userInfo.UserID, conversationID)
		if err != nil && errs.Unwrap(err) != redis.Nil {
			return 0, 0, nil, err
		}
		var cacheDelNum int
		...
		log.ZDebug(ctx, "get delSeqs from redis", "delSeqs", delSeqs, "userID", userInfo.UserID, "conversationID", conversationID, "cacheDelNum", cacheDelNum)
	...
	if len(failedSeqs) != 0 {
		log.ZDebug(ctx, "msgs not exist in redis", "seqs", failedSeqs, "cachedMsgs", cachedMsgs)
	}
	// get from cache or db
	if len(failedSeqs) > 0 {
		mongoMsgs, err := db.getMsgBySeqsRange(ctx, userInfo.UserID, conversationID, failedSeqs, begin, end, userInfo, groupMemberCache)
		if err != nil {
			return 0, 0, nil, err
		}
	...
	//将发送者的昵称改成备注昵称
	if friendsRemark != nil {
		for _, msg := range successMsgs {
			remarkTmp, existsTmp := friendsRemark[msg.SendID]
			if existsTmp {
				msg.SenderNickname = remarkTmp
			}
		}
	}
...
//		return 0, 0, nil, errs.ErrArgs.Wrap("seq end < begin")
//	}
//	//每次 获取 80 条消息的seq，最终只返回50条
//	seqs := utils.GetMsgSeqs(minSeq, end, pageMsgSize)
//	if len(seqs) == 0 {
//		log.ZWarn(ctx, "GetMsgBySeqsRange", nil, "userMinSeq", userMinSeq, "conMinSeq", minSeq, "conMaxSeq", maxSeq, "userMaxSeq", userMaxSeq, "begin", begin, "end", end)
//		return 0, 0, nil, nil
//	}
...
//	// get from cache or db
//	if len(failedSeqs) > 0 {
//		mongoMsgs, err := db.getMsgBySeqsRange(ctx, userInfo.UserID, conversationID, failedSeqs, minSeq, newEnd, userInfo, groupMemberCache)
//		if err != nil {
//			return 0, 0, nil, err
//		}
//		successMsgs = append(mongoMsgs, successMsgs...)
...
func (db *commonMsgDatabase) GetMsgBySeqs(ctx context.Context, userID string, conversationID string, seqs []int64,
	userInfo *sdkws.UserInfo, groupMemberCache *sdkws.GroupMemberFullInfo) (int64, int64, []*sdkws.MsgData, error) {
	...
	if len(failedSeqs) > 0 {
		mongoMsgs, err := db.getMsgBySeqs(ctx, userID, conversationID, failedSeqs, userInfo, groupMemberCache)
		if err != nil {
...
func (db *commonMsgDatabase) MsgIdGetConversations(ctx context.Context, req *pbmsg.MsgIdGetConversationsReq) (conversationIDs map[string]string, err error) {
	conversationIDs = make(map[string]string)
	failMsgIds := make([]string, 0)
	for _, msgId := range req.MsgId {
		conversationId, err := db.GetRevokeConversationIdExpire(ctx, msgId)
		if err != nil {
			failMsgIds = append(failMsgIds, msgId)
			continue
		}
		conversationIDs[msgId] = conversationId
	}
	if len(failMsgIds) > 0 {
		req.MsgId = failMsgIds
		conversationIdTemp, err := db.msgDocDatabase.MsgIdGetConversations(ctx, req)
		if err != nil {
			return nil, errs.Wrap(err)
		}
		if conversationIdTemp != nil {
			for msgIdTemp, tempId := range conversationIdTemp {
				conversationIDs[msgIdTemp] = tempId
				_ = db.SetRevokeConversationIdExpire(ctx, tempId, msgIdTemp)
			}
		}
	}
...
func (db *commonMsgDatabase) MsgIdGetConversationSeq(ctx context.Context, req *pbmsg.MsgIdGetConversationSeqReq) (resp *pbmsg.MsgIdGetConversationSeqResp, err error) {
	resp, err = db.msgDocDatabase.MsgIdGetConversationSeq(ctx, req)
	return resp, err
}
