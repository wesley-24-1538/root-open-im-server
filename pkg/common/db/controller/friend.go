...
type FriendDatabase interface {
	...
	// FindFriendsRemark 获取指定用户有设置备注的好友
	FindFriendsRemark(ctx context.Context, ownerUserID string) (friends map[string]string, err error)
}
...
// FindFriends 获取某人指定好友的信息 如果有好友不存在，不返回错误.
...
func (f *friendDatabase) FindFriendsRemark(ctx context.Context, ownerUserID string) (friends map[string]string, err error) {
	friendInfo := make(map[string]string)
	friendsRemark, err := f.friend.FindFriendsRemark(ctx, ownerUserID)
	if err != nil {
		return friendInfo, err
	}
	if len(friendsRemark) > 0 {
		for _, friendRemark := range friendsRemark {
			friendInfo[friendRemark.FriendUserID] = friendRemark.Remark
		}
	}
