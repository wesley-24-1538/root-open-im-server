...
func (m *MsgMongoDriver) GetMsgBySeqIndexIn1Doc(
	...
	for i := range msgDocModel[0].Msg {
		infoModel := msgDocModel[0].Msg[i]
		if infoModel == nil || infoModel.Msg == nil {
			continue
		}
		if infoModel.Revoke != nil {
			revokeContent := sdkws.MessageRevokedContent{
				RevokerID:                   infoModel.Revoke.UserID,
				RevokerRole:                 infoModel.Revoke.Role,
				ClientMsgID:                 infoModel.Msg.ClientMsgID,
				RevokerNickname:             infoModel.Revoke.Nickname,
				RevokeTime:                  infoModel.Revoke.Time,
				SourceMessageSendTime:       infoModel.Msg.SendTime,
				SourceMessageSendID:         infoModel.Msg.SendID,
				SourceMessageSenderNickname: infoModel.Msg.SenderNickname,
				SessionType:                 infoModel.Msg.SessionType,
				Seq:                         infoModel.Msg.Seq,
				Ex:                          infoModel.Msg.Ex,
			}
			...
			infoModel.Msg.ContentType = constant.MsgRevokeNotification
			infoModel.Msg.Content = string(content)
		}
		msgs = append(msgs, infoModel)
	}
	return msgs, nil
}
...
func (m *MsgMongoDriver) searchMessage(ctx context.Context, req *msg.SearchMessageReq) (int32, []*table.MsgInfoModel, error) {
	...
	if req.RecvID != "" {
		if req.SessionType == 3 || req.SessionType == 2 {
			condition = append(condition, bson.M{"$or": []bson.A{
				{"$$item.msg.group_id", req.RecvID},
				{"$$item.msg.recv_id", req.RecvID},
			}})
		} else {
			condition = append(condition, bson.M{"$regexFind": bson.M{"input": "$$item.msg.recv_id", "regex": regexp.QuoteMeta(req.RecvID), "$options": "i"}})
		}
	}
	if req.SendID != "" {
		condition = append(condition, bson.M{"$regexFind": bson.M{"input": "$$item.msg.send_id", "regex": regexp.QuoteMeta(req.SendID), "$options": "i"}})
	}
...
func (m *MsgMongoDriver) MsgIdGetConversations(ctx context.Context, req *msg.MsgIdGetConversationsReq) (map[string]string, error) {
	msgIdStr := "$$item.msg.server_msg_id"
	if req.IdType == 1 {
		msgIdStr = "$$item.msg.client_msg_id"
	}
	pipeline := []bson.M{
		{
			"$match": bson.M{
	...
	for _, msgId := range req.MsgId {
		for _, doc := range msgsDocs {
			if (req.IdType == 1 && msgId == doc.Msg.Msg.ClientMsgID) || (req.IdType == 2 && msgId == doc.Msg.Msg.ServerMsgID) {
				docIDArr := strings.Split(doc.DocID, ":")
				if len(docIDArr) >= 1 {
					conversationIds[msgId] = docIDArr[0]
				}
			}
		}
	}
...
func (m *MsgMongoDriver) MsgIdGetConversationSeq(ctx context.Context, req *msg.MsgIdGetConversationSeqReq) (*msg.MsgIdGetConversationSeqResp, error) {
	msgIdStr := "$$item.msg.server_msg_id"
	if req.IdType == 1 {
		msgIdStr = "$$item.msg.client_msg_id"
	}
	...
	log.ZDebug(ctx, "query mongoDB", "result", msgsDocs, "pipeline", pipeline)
	var resp msg.MsgIdGetConversationSeqResp
	for _, doc := range msgsDocs {
		if (req.IdType == 1 && req.MsgId == doc.Msg.Msg.ClientMsgID) || (req.IdType == 2 && req.MsgId == doc.Msg.Msg.ServerMsgID) {
			docIDArr := strings.Split(doc.DocID, ":")
			if len(docIDArr) >= 1 {
				resp.ConversationID = docIDArr[0]
			}
			resp.Seq = doc.Msg.Msg.Seq
		}
	}
	return &resp, nil
}

func (m *MsgMongoDriver) GetMsgByConversationId(ctx context.Context, userID string, conversationId string, conversationType int32, startSeq, endSeq int64, userAuth int, roleLevel int32) (msgs []*table.MsgInfoModel, err error) {
	var pipeline []bson.M
	condition := bson.A{}
	if userAuth == 0 && roleLevel == 20 {
		condition = append(condition, bson.M{"$eq": bson.A{"$$item.revoke", nil}})
		condition = append(condition, bson.M{"$ne": bson.A{"$$item.msg.content_type", 1508}})
	}
	regexStr := ":(1208)"
	condition = append(condition, bson.M{"$not": bson.M{"$regexFind": bson.M{"input": "$$item.msg.content", "regex": regexStr, "options": "i"}}})
	if conversationType == constant.SingleChatType {
		...
		condition = append(condition, bson.M{
			"$eq": bson.A{
				"$$item.msg.session_type",
				conversationType,
			},
		})
		condition = append(condition, bson.M{
			"$not": bson.M{
				"$in": bson.A{userID, "$$item.del_list"},
			},
		})
		pipeline = []bson.M{
			{
	...
	} else if conversationType == constant.SuperGroupChatType {
		condition = append(condition, bson.M{
			"$eq": bson.A{
				"$$item.msg.group_id",
				conversationId,
	...
	log.ZDebug(ctx, "query mongoDB", "result", msgDocModel, "pipeline", pipeline)
	msgs = make([]*table.MsgInfoModel, 0, len(msgDocModel))
	for _, msgDoc := range msgDocModel {
		if msgDoc.Msg == nil {
			continue
		}
		if msgDoc.Msg.Revoke != nil {
			revokeContent := sdkws.MessageRevokedContent{
				RevokerID:                   msgDoc.Msg.Revoke.UserID,
				RevokerRole:                 msgDoc.Msg.Revoke.Role,
				ClientMsgID:                 msgDoc.Msg.Msg.ClientMsgID,
				RevokerNickname:             msgDoc.Msg.Revoke.Nickname,
				RevokeTime:                  msgDoc.Msg.Revoke.Time,
				SourceMessageSendTime:       msgDoc.Msg.Msg.SendTime,
				SourceMessageSendID:         msgDoc.Msg.Msg.SendID,
				SourceMessageSenderNickname: msgDoc.Msg.Msg.SenderNickname,
				SessionType:                 msgDoc.Msg.Msg.SessionType,
				Seq:                         msgDoc.Msg.Msg.Seq,
				Ex:                          msgDoc.Msg.Msg.Ex,
			}
			data, err := json.Marshal(&revokeContent)
			if err != nil {
				return nil, err
			...
			msgDoc.Msg.Msg.ContentType = constant.MsgRevokeNotification
			msgDoc.Msg.Msg.Content = string(content)
		}
		msgs = append(msgs, msgDoc.Msg)
	}
	return msgs, err
}
